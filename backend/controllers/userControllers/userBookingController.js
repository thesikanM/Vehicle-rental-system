...
export const BookCar = async (req, res, next) => {
  try {
    if (!req.body) {
      return next(errorHandler(400, "Bad request: no body provided"));
    }
    ...
    if (new Date(pickupDate) >= new Date(dropoffDate)) {
      return next(errorHandler(400, "Invalid date range: pickup must be before dropoff"));
    }

    // Validate pickup date is not in the past
    if (new Date(pickupDate) < new Date()) {
      return next(errorHandler(400, "Pickup date cannot be in the past"));
    }
...
export const razorpayOrder = async (req, res, next) => {
  try {
    ...
    if (!order) return next(errorHandler(500, "Failed to create Razorpay order"));
    res.status(200).json(order);
  } catch (error) {
    next(errorHandler(500, "Error creating Razorpay order"));
  }
};
...
export const sendBookingDetailsEamil = async (req, res, next) => {
  try {
    ...
    if (!toEmail || !data) {
      return next(errorHandler(400, "Missing email or booking data"));
    }
